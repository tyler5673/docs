...
To make your graph rebuild on each new run with custom configuration, you need to rewrite `openai_agent.py` to instead provide an _async context manager_ that takes a config and yields a graph (or compiled graph) instance. Let's say we want to return our existing graph for user ID '1', and a tool-calling agent for other users. We can modify `openai_agent.py` as follows:
...
async def make_graph(config: RunnableConfig):
    ...
    if user_id == "1":
        yield make_default_graph()
    else:
        yield make_alternative_graph()
```

Finally, you need to specify the path to your graph-making async context manager (`make_graph`) in `langgraph.json`:
