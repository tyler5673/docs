...
- Return a [`Command`](https://reference.langchain.com/javascript/classes/_langchain_langgraph.index.Command.html) from `wrap_model_call` / `wrapModelCall` to inject state updates from the model call layer (Python: use @[`ExtendedModelResponse`]; JavaScript: return `Command` directly). See [State updates from wrap_model_call](/oss/javascript/langchain/middleware/custom#state-updates-from-wrap_model_call).
- Use life-cycle hooks like `before_model` or `after_model` to update the conversation history. See the [middleware documentation](/oss/javascript/langchain/middleware) for more details.
</Note>
