...
- Return a [`Command`](https://reference.langchain.com/python/langgraph/types/#langgraph.types.Command) from `wrap_model_call` / `wrapModelCall` to inject state updates from the model call layer (Python: use [`ExtendedModelResponse`](https://reference.langchain.com/python/langchain/middleware/#langchain.agents.middleware.ExtendedModelResponse); JavaScript: return `Command` directly). See [State updates from wrap_model_call](/oss/python/langchain/middleware/custom#state-updates-from-wrap_model_call).
- Use life-cycle hooks like `before_model` or `after_model` to update the conversation history. See the [middleware documentation](/oss/python/langchain/middleware) for more details.
</Note>
