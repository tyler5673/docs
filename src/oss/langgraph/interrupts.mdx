...
- <Icon icon="link" /> [Map interrupt IDs to resume values](#map-interrupt-ids-to-resume-values): Pair interrupt IDs with resume values when resuming multiple interrupts in a single invocation
...
def ask_pending_hitl_questions(interrupts: list) -> dict[str, str] | None:
    resume_map = {}
    for pending_interrupt in interrupts:
        answer = input(f"Q: {pending_interrupt.value} ")
        resume_map[pending_interrupt.id] = answer
    return resume_map or None

# In your run loop, when the graph has interrupted:
resume_map = ask_pending_hitl_questions(state)
if resume_map:
    async for e in workflow.astream(Command(resume=resume_map), config=config):
        print(f"===> {e}")
else:
    break
```
...
async function askPendingHitlQuestions(
  state: StateSnapshot,
): Promise<Record<string, string> | null> {
  const rl = createInterface({ input, output });
  const resumeMap: Record<string, string> = {};
  for (const pendingInterrupt of getPendingInterrupts(state)) {
    const answer = await rl.question(`Q: ${pendingInterrupt.value as string} `);
    const id = pendingInterrupt.id ?? randomUUID();
    resumeMap[id] = answer;
  }
  rl.close();
  return Object.keys(resumeMap).length > 0 ? resumeMap : null;
}

// When the graph has interrupted, build the resume map from result.__interrupt__
const resumeMap = await askPendingHitlQuestions(state);
if (resumeMap) {
  const resumeStream = await workflow.stream(
  new Command({ resume: resumeMap }),
    config,
  );
  for await (const e of resumeStream) {
