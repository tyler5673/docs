...
- Return a @[`Command`] from `wrap_model_call` / `wrapModelCall` to inject state updates from the model call layer (Python: use @[`ExtendedModelResponse`]; JavaScript: return `Command` directly). See [State updates from wrap_model_call](/oss/langchain/middleware/custom#state-updates-from-wrap_model_call).
